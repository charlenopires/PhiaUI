defmodule PhiaUi.Components.Editor.TipTapTest do
  ...
  defmodule H do
    ...
    import PhiaUi.Components.Editor.TipTap

    # tiptap_editor
    def render_tiptap_editor(assigns) do
    ...
    # tiptap_toolbar
    def render_tiptap_toolbar(assigns) do
    ...
    # tiptap_content
    def render_tiptap_content(assigns) do
      assigns = Map.put_new(assigns, :content, nil)
      assigns = Map.put_new(assigns, :prose_size, :base)
      assigns = Map.put_new(assigns, :class, nil)

      ~H"""
      <.tiptap_content content={@content} prose_size={@prose_size} class={@class} />
      """
    end

    # tiptap_bubble_menu
    def render_tiptap_bubble_menu(assigns) do
      assigns = Map.put_new(assigns, :id, "bubble-1")
      assigns = Map.put_new(assigns, :editor_id, "test-editor")
      assigns = Map.put_new(assigns, :class, nil)

      ~H"""
      <.tiptap_bubble_menu id={@id} editor_id={@editor_id} class={@class}>
        <button>Bold</button>
      </.tiptap_bubble_menu>
      """
    end
    ...
    def render_tiptap_floating_menu(assigns) do
      assigns = Map.put_new(assigns, :id, "float-1")
      assigns = Map.put_new(assigns, :editor_id, "test-editor")
      assigns = Map.put_new(assigns, :class, nil)

      ~H"""
      <.tiptap_floating_menu id={@id} editor_id={@editor_id} class={@class}>
    ...
    def render_tiptap_slash_commands(assigns) do
      assigns = Map.put_new(assigns, :id, "slash-1")
      assigns = Map.put_new(assigns, :editor_id, "test-editor")
      assigns = Map.put_new(assigns, :class, nil)

      ~H"""
      <.tiptap_slash_commands id={@id} editor_id={@editor_id} class={@class}>
    ...
    # tiptap_mention
    def render_tiptap_mention(assigns) do
    ...
    def render_tiptap_collab_cursors(assigns) do
  ...
  describe "tiptap_editor/1" do
    test "renders with required id" do
      html = render_component(&H.render_tiptap_editor/1, %{id: "my-editor"})
      assert html =~ ~s[id="my-editor"]
      assert html =~ ~s[phx-hook="PhiaTipTap"]
    end

    test "renders hidden input for form value" do
      html = render_component(&H.render_tiptap_editor/1, %{id: "ed"})
      assert html =~ ~s[type="hidden"]
      assert html =~ ~s[data-tiptap-hidden]
    end

    test "renders content area with data-tiptap-content" do
      html = render_component(&H.render_tiptap_editor/1, %{id: "ed"})
      assert html =~ ~s[data-tiptap-content]
    end

    test "renders placeholder data attr" do
      html =
        render_component(&H.render_tiptap_editor/1, %{id: "ed", placeholder: "Write something"})
    ...
    test "renders word count footer by default" do
      html = render_component(&H.render_tiptap_editor/1, %{id: "ed"})
      assert html =~ "data-tiptap-footer"
      assert html =~ "data-tiptap-word-count"
    end

    test "hides word count when show_word_count is false" do
      html = render_component(&H.render_tiptap_editor/1, %{id: "ed", show_word_count: false})
      refute html =~ "data-tiptap-footer"
    end
    ...
    test "renders data-on-update event name" do
      html = render_component(&H.render_tiptap_editor/1, %{id: "ed", on_update: "content:changed"})
      assert html =~ ~s[data-on-update="content:changed"]
    end

    test "renders data-on-selection event name" do
      html =
        render_component(&H.render_tiptap_editor/1, %{id: "ed", on_selection: "sel:changed"})

      assert html =~ ~s[data-on-selection="sel:changed"]
    end

    test "renders data-content-format" do
      html = render_component(&H.render_tiptap_editor/1, %{id: "ed", content_format: :html})
      assert html =~ ~s[data-content-format="html"]
    end

    test "renders min-height style" do
      html = render_component(&H.render_tiptap_editor/1, %{id: "ed", min_height: "500px"})
      assert html =~ "min-height: 500px"
    end

    test "renders custom class" do
      html = render_component(&H.render_tiptap_editor/1, %{id: "ed", class: "my-custom"})
      assert html =~ "my-custom"
    end

    test "renders extensions as JSON data attr" do
      html =
        render_component(&H.render_tiptap_editor/1, %{
          id: "ed",
  ...
  describe "tiptap_toolbar/1" do
    test "renders toolbar role" do
      html = render_component(&H.render_tiptap_toolbar/1, %{editor_id: "ed"})
      assert html =~ ~s[role="toolbar"]
    end
    ...
    test "marks active bold button" do
      html =
        render_component(&H.render_tiptap_toolbar/1, %{
          editor_id: "ed",
          active_marks: ["bold"]
        })
    ...
    test "marks inactive bold button" do
      html =
        render_component(&H.render_tiptap_toolbar/1, %{editor_id: "ed", active_marks: []})
  ...
  describe "tiptap_content/1" do
    test "renders empty for nil content" do
      html = render_component(&H.render_tiptap_content/1, %{content: nil})
      assert html =~ "prose"
    end
    ...
    test "renders JSON content as HTML" do
      ...
      html = render_component(&H.render_tiptap_content/1, %{content: content})
    ...
    test "renders prose-sm variant" do
      html = render_component(&H.render_tiptap_content/1, %{content: nil, prose_size: :sm})
      assert html =~ "prose-sm"
    end

    test "renders prose-lg variant" do
      html = render_component(&H.render_tiptap_content/1, %{content: nil, prose_size: :lg})
  ...
  describe "tiptap_bubble_menu/1" do
    test "renders with id and editor_id" do
      html = render_component(&H.render_tiptap_bubble_menu/1, %{id: "bub", editor_id: "ed"})
      assert html =~ ~s[id="bub"]
      assert html =~ ~s[data-editor-id="ed"]
      assert html =~ ~s[data-tiptap-bubble-menu]
    end
  ...
  describe "tiptap_floating_menu/1" do
    test "renders with id and editor_id" do
      html =
        render_component(&H.render_tiptap_floating_menu/1, %{id: "float", editor_id: "ed"})

      assert html =~ ~s[id="float"]
      assert html =~ ~s[data-editor-id="ed"]
      assert html =~ ~s[data-tiptap-floating-menu]
    end
  ...
  describe "tiptap_slash_commands/1" do
    test "renders with id and editor_id" do
      html =
        render_component(&H.render_tiptap_slash_commands/1, %{id: "slash", editor_id: "ed"})

      assert html =~ ~s[id="slash"]
      assert html =~ ~s[data-editor-id="ed"]
      assert html =~ ~s[data-tiptap-slash-commands]
    end
  ...
  describe "tiptap_mention/1" do
    test "renders with id and editor_id" do
      html = render_component(&H.render_tiptap_mention/1, %{id: "men", editor_id: "ed"})
      assert html =~ ~s[id="men"]
      assert html =~ ~s[data-editor-id="ed"]
      assert html =~ ~s[data-tiptap-mention]
    end
    ...
    test "renders suggestions" do
      ...
      html =
        render_component(&H.render_tiptap_mention/1, %{
          id: "men",
          editor_id: "ed",
          suggestions: suggestions
        })
  ...
  describe "tiptap_collab_cursors/1" do
    test "renders with id and editor_id" do
      html =
        render_component(&H.render_tiptap_collab_cursors/1, %{id: "cur", editor_id: "ed"})
    ...
    test "renders empty when no cursors" do
      html =
        render_component(&H.render_tiptap_collab_cursors/1, %{
          id: "cur",
          editor_id: "ed",
          cursors: []
        })
