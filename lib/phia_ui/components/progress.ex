defmodule PhiaUi.Components.Progress do
  ...
  attr(:max, :integer,
    default: 100,
    doc: "Maximum value for the progress bar. Default: `100`. Set to a custom value when your scale differs (e.g. `max={5}` for a 5-step flow)."
  )

  attr(:class, :string,
    default: nil,
    doc: "Additional CSS classes applied to the track element. Use `h-N` to change bar height (default `h-2`)."
  )

  attr(:rest, :global, doc: "HTML attributes forwarded to the root element. Useful for `aria-label` or `aria-labelledby`.")
