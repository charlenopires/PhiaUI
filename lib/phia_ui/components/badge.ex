defmodule PhiaUi.Components.Badge do
  ...
  attr(:variant, :atom,
    ...
    default: :default,
    doc: "Visual style variant. Controls background, text colour, and hover state. Default: `:default` (solid primary)."
  )

  attr(:class, :string,
    default: nil,
    doc: "Additional CSS classes merged via `cn/1`. Use `uppercase tracking-wide` for label-style badges, or `gap-1` to add an icon."
  )

  attr(:rest, :global, doc: "HTML attributes forwarded to the root `<div>` element (e.g. `data-testid`, `aria-label`).")

  slot(:inner_block, required: true, doc: "Badge content. Typically a short text label (1–3 words), count, or an icon + text combination.")
