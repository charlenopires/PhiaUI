defmodule PhiaUi.Components.Icon do
  ...
  attr(:name, :string,
    required: true,
    doc: "Lucide icon name in kebab-case (e.g. `\"menu\"`, `\"chevron-down\"`, `\"trending-up\"`). See https://lucide.dev/icons for all available names."
  )

  attr(:size, :atom,
    values: [:xs, :sm, :md, :lg],
    default: :md,
    doc: "Icon size atom. Maps to Tailwind classes: `:xs` → `w-3 h-3`, `:sm` → `w-4 h-4`, `:md` → `w-5 h-5`, `:lg` → `w-6 h-6`."
  )

  attr(:class, :string,
    default: nil,
    doc: "Additional CSS classes merged via `cn/1` (last wins). Use `text-*` for colour, `mr-N` / `ml-N` for spacing in flex containers."
  )
