defmodule PhiaUi.Components.AspectRatio do
  ...
  **width**, setting `padding-top` to `1/ratio * 100%` creates a box whose
  ...
  attr(:ratio, :float,
    default: 1.0,
    doc: "Aspect ratio expressed as `width / height`. Default `1.0` (square). Common values: `16/9`, `4/3`, `21/9`, `9/16`."
  )

  attr(:class, :string,
    default: nil,
    doc: "Additional CSS classes applied to the outer wrapper `<div>`. Use `max-w-N`, `rounded-*`, and `overflow-hidden` here."
  )
  ...
  slot(:inner_block, required: true, doc: "Content to render inside the aspect-ratio-constrained space. Set `class=\"w-full h-full\"` on child elements to fill the box.")
