defmodule PhiaUi.Components.ButtonGroup do
  ...
  attr(:orientation, :string,
    ...
    doc:
      "Group layout direction: `\"horizontal\"` (default, side-by-side) or `\"vertical\"` (stacked)."
  )

  attr(:class, :string,
    default: nil,
    doc:
      "Additional CSS classes merged via `cn/1`. Use `w-full` to stretch the group to full width."
  )
  ...
  slot(:inner_block,
    required: true,
    doc:
      "Button components to group. Any `PhiaUi.Components.Button` variant or plain `<button>` works."
  )
