defmodule PhiaUi.Components.EmptyState do
  ...
  attr(:class, :string,
    default: nil,
    doc:
      "Additional CSS classes applied to the outer container `<div>`. Use `min-h-[N]` to control minimum height."
  )
  ...
  slot(:icon,
    doc:
      "Optional leading icon or illustration. Rendered in `text-muted-foreground` with bottom margin."
  )

  slot(:title,
    required: true,
    doc: "Primary heading text for the empty state. Rendered as a bold `<h3>`."
  )

  slot(:description,
    doc:
      "Optional supporting text with additional context. Rendered in muted small type, max width constrained for readability."
  )

  slot(:action,
    doc: "Optional call-to-action area. Place a button, link, or any interactive element here."
  )
