defmodule PhiaUi.Components.Resizable do
  ...
  attr(:class, :string,
    default: nil,
    doc:
      "Additional CSS classes applied to the container `<div>`. Always set an explicit height (e.g. `class=\"h-[400px]\"`) so panels have space to fill."
  )
  ...
  attr(:class, :string,
    default: nil,
    doc:
      "Additional CSS classes applied to the panel `<div>`. Typically `overflow-auto` or `overflow-hidden`."
  )
  ...
  attr(:class, :string,
    default: nil,
    doc: "Additional CSS classes applied to the handle `<div>`."
  )
