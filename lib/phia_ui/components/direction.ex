defmodule PhiaUi.Components.Direction do
  ...
  attr(:dir, :string,
    default: "ltr",
    values: ~w(ltr rtl),
    doc: "Text direction. `\"ltr\"` (default) for left-to-right languages (English, French, etc.). `\"rtl\"` for right-to-left languages (Arabic, Hebrew, Farsi, Urdu)."
  )
  ...
  attr(:rest, :global, doc: "HTML attributes forwarded to the root `<div>` element (e.g. `lang`, `data-*`).")

  slot(:inner_block, required: true, doc: "Content inside the direction wrapper. All descendant text and inline elements respect the `dir` attribute.")
