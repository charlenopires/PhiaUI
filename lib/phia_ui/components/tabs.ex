defmodule PhiaUi.Components.Tabs do
  ...
  attr(:active, :string,
    ...
    selected state (`bg-background shadow-sm`). When using `:let={ctx}` on
    `tabs/1`, this is populated automatically.
    """
  )
  ...
  def tabs_trigger(assigns) do
    ...
    <button
      ...
      class={cn([
        ...
        # Active state: white background with shadow, full-contrast text
        @selected && "bg-background text-foreground shadow-sm",
        # Inactive state: no background, muted text, subtle hover
        !@selected && "hover:bg-background/50",
        # Disabled: prevent interaction and dim the trigger
